...
local function agent(id, status, workspace, revision, cwd)
  return {
    terminal_id = id,
    target = id,
    status = status,
...
h.test("state shows the current project plus attention elsewhere", function()
  reset()
  state._set_transition_handler(function() end)
  state._set_emit(function() end)
  state._accept(snapshot({
    agent("local", "working", "w1", 1, "/repo/src"),
    agent("remote-working", "working", "w2", 1, "/other"),
    agent("remote-blocked", "blocked", "w2", 1, "/other"),
  }))
  local groups = state.grouped_agents({ root = "/repo" })
  h.eq(
    { "local" },
    vim.tbl_map(function(item)
      return item.name
