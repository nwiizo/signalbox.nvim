...
local transition_handler = function(event)
  require("signalbox.notifier").on_transition(event)
end
...
local function emit_transition(agent, previous_status)
  ...
  emit_user("SignalboxAgentStatusChanged", event)
end
...
local function finish_snapshot_waiters(ok, result)
  local waiters = snapshot_waiters
  snapshot_waiters = {}
...
function M._accept(snapshot)
  ...
  initialized = true
  emit_user("SignalboxUpdated", { stale = false, counts = counts(snapshot.agents) })
  finish_snapshot_waiters(true, snapshot)
end
...
function M._fail(err, opts)
  ...
  if err and err.kind == "executable" and not failure_notifications.executable then
    ...
    vim.notify(
      "Herdr executable was not found. Install Herdr from https://herdr.dev/ and run :checkhealth signalbox",
      vim.log.levels.ERROR,
      { title = "Signalbox" }
    )
  end
  emit_user("SignalboxUpdated", { stale = stale, error = err })
  finish_snapshot_waiters(false, err)
end
...
function M.ensure_snapshot(callback)
  if current and not stale then
    callback(true, current)
    return
  end
  table.insert(snapshot_waiters, callback)
...
local function same_project(agent_cwd, root)
  if not agent_cwd or not root then
    return false
  end
  local agent_path = vim.fn.fnamemodify(agent_cwd, ":p")
  local root_path = vim.fn.fnamemodify(root, ":p")
  agent_path = agent_path:gsub("/$", "")
  root_path = root_path:gsub("/$", "")
  agent_path = vim.fs.normalize(agent_path)
  root_path = vim.fs.normalize(root_path)
  return agent_path == root_path or agent_path:sub(1, #root_path + 1) == root_path .. "/"
end

function M.grouped_agents(opts)
  opts = opts or {}
  if not current then
    return {}
  end
  ...
  for index, agent in ipairs(current.agents) do
    ...
    local visible = opts.all == true
      or opts.root == nil
      or same_project(agent.cwd, opts.root)
      or attention_status[agent.status]
    if visible then
...
function M.stop()
  ...
  timer = nil
  finish_snapshot_waiters(false, { kind = "lifecycle", message = "Signalbox state stopped before snapshot" })
end
