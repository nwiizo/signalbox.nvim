...
function M.attach(agent, opts)
      ...
      vim.schedule(function()
        ...
        if unexpected and code ~= 0 and vim.v.exiting == vim.NIL then
          ...
          if lower:find("takeover", 1, true) or lower:find("another client", 1, true) then
            vim.notify(
              string.format(
                "Herdr attach for %s is owned by another client; use :SignalboxAttach! to take over",
                agent.name
              ),
              vim.log.levels.WARN,
              { title = "Signalbox" }
            ...
            vim.notify(
              string.format("Herdr attach for %s exited with code %d; run :SignalboxHealth", agent.name, code),
              vim.log.levels.WARN,
              { title = "Signalbox" }
            )
          end
  ...
  if not job_id then
    ...
    vim.notify(attach_err.message, vim.log.levels.ERROR, { title = "Signalbox" })
    return nil
  end
